            ...
            <c:forEach var="leftCategory" items="${applicationScope.LEFT_CATEGORY_LST }">
              <c:choose>
                <c:when
                  ...
                  <li class="dropdown">
                    <a href="/${leftCategory.controllerName}/${leftCategory.urlName}" class="dropdown-toggle"
                        ...
                        <c:choose>
                          <c:when test="${subLeftCategory.grouped == 1}">
                            ...
                            <c:forEach var="lastLeftCategory" items="${subLeftCategory.subCategoryVoList}">
                              <li><a
                                href="/${lastLeftCategory.controllerName}/${lastLeftCategory.urlName}">${lastLeftCategory.displayName}</a>
                              </li>
                            </c:forEach>
                          </c:when>
                          <c:otherwise>
                            <li><a
                              href="/${subLeftCategory.controllerName}/${subLeftCategory.urlName}">${subLeftCategory.displayName}</a>
                            </li>
                          </c:otherwise>
                        </c:choose>
                      </c:forEach>
                ...
                <c:otherwise>
                  <li><a href="/${leftCategory.controllerName}/${leftCategory.urlName}">${leftCategory.displayName}</a>
                  </li>
                </c:otherwise>
              </c:choose>
            </c:forEach>
          ...
          <form class="navbar-form navbar-right" role="search" method="post" action="/search">
            <div class="input-group">
              <input type="text" class="form-control" id="search" name="search"
                     placeholder="${applicationScope.CONFIG_MAP["ROC_DESC"]}" required>
            ...
            <c:forEach var="rightCategory" items="${applicationScope.RIGHT_CATEGORY_LST}">
              <c:choose>
                <c:when
                  ...
                  <li class="dropdown">
                    <a href="/${rightCategory.controllerName}/${rightCategory.urlName}" class="dropdown-toggle"
                       data-toggle="dropdown" role="button" aria-expanded="false">
                        ...
                        <c:choose>
                          <c:when test="${subRightCategory.grouped == 1}">
                            ...
                            <c:forEach var="lastRightCategory" items="${subRightCategory.subCategoryVoList}">
                              <li><a
                                href="/${lastRightCategory.controllerName}/${lastRightCategory.urlName}">${lastRightCategory.displayName}</a>
                              </li>
                            </c:forEach>
                          </c:when>
                          <c:otherwise>
                            <li><a
                              href="/${subRightCategory.controllerName}/${subRightCategory.urlName}">${subRightCategory.displayName}</a>
                            </li>
                          </c:otherwise>
                        </c:choose>
                      </c:forEach>
                ...
                <c:otherwise>
                  <li><a
                    href="/${rightCategory.controllerName}/${rightCategory.urlName}">${rightCategory.displayName}</a>
                  </li>
                </c:otherwise>
              </c:choose>
            </c:forEach>
