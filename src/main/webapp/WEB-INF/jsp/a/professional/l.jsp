              ...
              <table class="table table-bordered table-hover table-condensed" style="border-bottom: solid 1px #ddd">
                ...
                <c:forEach var="professionalVo" items="${professionalVoList}">
                  <tr>
                    ...
                    <td><fmt:formatDate value="${professionalVo.addDate}" type="both"
                                        pattern="yyyy-MM-dd HH:mm:ss"/></td>
                    <td><fmt:formatDate value="${professionalVo.rmvDate}" type="both"
                                        pattern="yyyy-MM-dd HH:mm:ss"/></td>
                    <td><input type="checkbox" disabled
                               <c:if test="${professionalVo.visible == 1}">checked="checked"</c:if> /></td>
                    <td><input type="checkbox" disabled
                               <c:if test="${professionalVo.operable == 1}">checked="checked"</c:if> /></td>
                    <td><input type="checkbox" disabled
                               <c:if test="${professionalVo.deleted == 1}">checked="checked"</c:if> /></td>
                    <td><a href="/a/professional/d/${professionalVo.id}">RMV</a></td>
                  </tr>
                </c:forEach>
              </table>
            </div>
