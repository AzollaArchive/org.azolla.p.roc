        ...
        <article class="roc-c-article-post">
          <div class="panel panel-default">
            ...
            <div class="panel-heading">
              ${jsp_title} <span class="label label-default"><a href="/a/c/m" style="color: #FFFFFF;text-shadow: none">ADD</a></span>
            </div>
            <!-- Table -->
                ...
                <c:forEach var="categoryVo" items="${categoryVoList}">
                  <tr>
                    ...
                    <td><input type="checkbox" disabled
                               <c:if test="${categoryVo.visible == 1}">checked="checked"</c:if> /></td>
                    <td><input type="checkbox" disabled
                               <c:if test="${categoryVo.operable == 1}">checked="checked"</c:if> /></td>
                    <td><input type="checkbox" disabled
                               <c:if test="${categoryVo.deleted == 1}">checked="checked"</c:if> /></td>
                    <td><a href="/a/c/m/${categoryVo.id}">MOD</a></td>
                  </tr>
                </c:forEach>
              </table>
            </div>
